...
export default function Contact() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!EMAILJS_SERVICE_ID || !EMAILJS_TEMPLATE_ID || !EMAILJS_PUBLIC_KEY) {
      setStatus("error");
      setErrorMessage(
        "Email isn't configured yet — add your EmailJS keys to .env (see .env.example).",
      );
      return;
    }
  ...
  return (
    <section id="contact" className="py-24 md:py-32 relative scroll-mt-24">
      {/* Background glow */}
      <div
        className="absolute right-0 bottom-0 w-96 h-96 rounded-full blur-[200px] pointer-events-none"
        style={{ background: "hsl(var(--secondary) / 0.06)" }}
      />

      <div className="container mx-auto px-5 sm:px-6 lg:px-8">
        <SectionHeading
          eyebrow="N°06 / Get In Touch"
            ...
            {fields.map((f, i) => (
              ...
              >
                <label className="flex items-center gap-3 font-mono text-xs text-muted-foreground tracking-wider mb-3">
                  <span className="text-primary font-semibold">{f.num}</span>{" "}
                  {f.label}
                </label>
            ...
            >
              ...
              {status === "error" && (
                <motion.span
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="flex items-center gap-2 text-destructive text-sm font-mono max-w-sm"
                >
