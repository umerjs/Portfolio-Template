...
const primaryLogos = allSkills.slice(0, 14).map(t => ({
  node: <span className="font-mono text-muted-foreground/60 tracking-widest">{t}</span>,
}));
const secondaryLogos = [...allSkills.slice(14), ...allSkills.slice(0, 6)].map(t => ({
  node: <span className="font-mono text-primary/40 tracking-widest">{t}</span>,
}));
      ...
      <div className="container mx-auto px-5 sm:px-6 lg:px-8">
        ...
        <div className="space-y-3 mb-16">
          <div className="border-y border-border/50 py-4">
            <LogoLoop logos={primaryLogos} speed={60} logoHeight={20} gap={56} direction="left" />
          </div>
          <div className="border-b border-border/50 py-4">
            <LogoLoop logos={secondaryLogos} speed={45} logoHeight={20} gap={56} direction="right" />
          </div>
        </div>
          ...
          {categories.map((cat, i) => {
            ...
            >
              <TiltedCard
                ...
                overlayContent={
                  <div className="w-full h-[280px] p-6">
